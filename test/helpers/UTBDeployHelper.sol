...
contract UTBDeployHelper is RouterActions, UniswapperHelpers {
    ...
    mapping(string => UTBFeeManager) feeManagerLookup;
    ...
    function deployFeeManager(
        string memory chain,
        address payable utb
    ) public returns (UTBFeeManager feeManager) {
        feeManager = UTBFeeManager(
            payable(
                deployChain(
                    chain,
                    "UTBFeeManager",
                    "UTBFeeManager.sol:UTBFeeManager",
                    ""
                )
            )
        );

        feeManager.setSigner(vm.addr(SIGNER_PRIVATE_KEY));
        UTB(utb).setFeeManager(payable(address(feeManager)));
        feeManagerLookup[chain] = feeManager;
        dump();
    }
        ...
        )
    {
        ...
        deployFeeManager(chain, payable(utb));
    ...
    function setupXChainUTBInfraReturnSrcUTB(string memory src, string memory dst) public returns (UTB srcUtb) {
        (srcUtb,,,,,) = deployUTBAndItsComponents(src);
        deployUTBAndItsComponents(dst);
        wireUpXChainUTB(src, dst);
    }
}
