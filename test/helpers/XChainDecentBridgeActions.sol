...
import {FeeData, Fee} from "../../src/CommonTypes.sol";
...
contract XChainDecentBridgeActions is UTBDeployHelper {
    ...
    uint16 constant STARGATE_MIN_SLIPPAGE = 1_00; // 1% BPS
    address feeToken = address(0);
    uint feeAmount = 0;
    address feeRecipientNative = address(0x1CE0FFEE);
    address feeRecipientUsdc = address(0xC0CAC01A);
    ...
    function getFeesAndSignature(
    ...
    ) public view returns (bytes memory signature, FeeData memory feeData) {
        Fee[] memory appFees = new Fee[](1);

        appFees[0] = Fee({
            recipient: feeToken == address(0) ? feeRecipientNative : feeRecipientUsdc,
            token: feeToken,
            amount: feeAmount
        });
        ...
        signature = getSignature(abi.encode(instructions, feeData));
    }
